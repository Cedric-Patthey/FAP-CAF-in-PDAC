...
# Normal pancreas from Olaniru et al
Integrated object is loaded
```{r}
...
Fig_S3AE_plot <- plot_grid(p1, p2, p3, p4, nrow = 1)
...
#Figure S3B
```{r}
Fig_S3B_plot <- ggplot(Selected.Norm.Panc.Cluster.Pathways, aes(x = Cluster, y = Pathway)) + 
...
if (!dir.exists("Figure_S3")) {dir.create("Figure_S3")}
ggsave(plot = Fig_S3B_plot, "Figure_S3B_Olaniru_Developing_Panc_GSEA.pdf", width = 7.5, height = 4.4444444, units = "in", path = "Figure_S3/")
```
...
The pathways enriched in FAP mesenchymal clusters are determined
```{r}
FAP.Mesenchymal.Souped.Singlets.rPCA.Integrated@meta.data <- FAP.Mesenchymal.Souped.Singlets.rPCA.Integrated@meta.data %>%
  mutate(Cluster.0.GSEA = case_when(
    integrated_snn_res.0.1 == 0 ~ "0",
...
```{r}
FAP.Mesenchymal.scpa_out_cluster_0 <- compare_seurat(FAP.Mesenchymal.Souped.Singlets.rPCA.Integrated,
                           group1 = "Cluster.0.GSEA", 
                           group1_population = c("0", "Other"),
                           pathways = pathways)
FAP.Mesenchymal.scpa_out_cluster_1 <- compare_seurat(FAP.Mesenchymal.Souped.Singlets.rPCA.Integrated,
                           group1 = "Cluster.1.GSEA", 
                           group1_population = c("1", "Other"),
                           pathways = pathways)
FAP.Mesenchymal.scpa_out_cluster_2 <- compare_seurat(FAP.Mesenchymal.Souped.Singlets.rPCA.Integrated,
                           group1 = "Cluster.2.GSEA", 
                           group1_population = c("2", "Other"),
                           pathways = pathways)
FAP.Mesenchymal.scpa_out_cluster_3 <- compare_seurat(FAP.Mesenchymal.Souped.Singlets.rPCA.Integrated,
                           group1 = "Cluster.3.GSEA", 
                           group1_population = c("3", "Other"),
                           pathways = pathways)
FAP.Mesenchymal.scpa_out_cluster_4 <- compare_seurat(FAP.Mesenchymal.Souped.Singlets.rPCA.Integrated,
...
```{r}
FAP.Mesenchymal.scpa_out_cluster_0_enriched <- FAP.Mesenchymal.scpa_out_cluster_0 %>%
  filter(FC > 0) %>%
  filter(adjPval < 0.05) %>%
  mutate(Cluster = "Cancer 0")

FAP.Mesenchymal.scpa_out_cluster_1_enriched <- FAP.Mesenchymal.scpa_out_cluster_1 %>%
  filter(FC > 0) %>%
  filter(adjPval < 0.05)  %>%
  mutate(Cluster = "Cancer 1")

FAP.Mesenchymal.scpa_out_cluster_2_enriched <- FAP.Mesenchymal.scpa_out_cluster_2 %>%
  filter(FC > 0) %>%
  filter(adjPval < 0.05) %>%
  mutate(Cluster = "Cancer 2")

FAP.Mesenchymal.scpa_out_cluster_3_enriched <- FAP.Mesenchymal.scpa_out_cluster_3 %>%
  filter(FC > 0) %>%
  filter(adjPval < 0.05) %>%
  mutate(Cluster = "Cancer 3")

FAP.Mesenchymal.scpa_out_cluster_4_enriched <- FAP.Mesenchymal.scpa_out_cluster_4 %>%
  filter(FC > 0) %>%
  filter(adjPval < 0.05) %>%
  top_n(100, qval) %>%
...
The pathways enriched in FAP+ mesenchymal cells are compared to the pathways enriched in Normal pancreas clusters determined above
```{r}
Top.Common.Norm.Cancer.Cluster.0 <- FAP.Mesenchymal.scpa_out_cluster_0_enriched %>%
...
Top.Common.Norm.Cancer.Cluster.1 <- FAP.Mesenchymal.scpa_out_cluster_1_enriched %>%
  inner_join(Norm.Panc.scpa_out_cluster_1_enriched, by = "Pathway") %>%
  mutate(qval.combined = (qval.x + FC.x) + (qval.y + FC.y)) %>%
  top_n(20, qval.combined)

Top.Common.Norm.Cancer.Cluster.3 <- FAP.Mesenchymal.scpa_out_cluster_3_enriched %>%
...
Common.Norm.Cancer.Pathways <- rbind(FAP.Mesenchymal.scpa_out_cluster_0_enriched, FAP.Mesenchymal.scpa_out_cluster_1_enriched, FAP.Mesenchymal.scpa_out_cluster_3_enriched, Norm.Panc.scpa_out_cluster_0_enriched, Norm.Panc.scpa_out_cluster_1_enriched, Norm.Panc.scpa_out_cluster_3_enriched)
