---
title: "PSC_cocul_day6_11s_scSeq_fastq_to_count"
author: "Cedric"
date: "2025-02-10"
...
# Summary 
The following code describes the processing of single cell RNAseq data from fastq files to raw UMI count for the ddSeq data generated in the co-culture experiment depicted in figure 3 of Cumming et al, 2025.
The input data consists of 11 paired-end fastq files corresponding to 11 samples. 
The raw sequence data as well as raw umi count is available on ArrayExpress with acession number E-MTAB-14944. 
...
The pipeline is run on the 11 samples
```{bash}
i=1
while [ $i -le 11 ]
do
  sbatch ddSeq_fastq_to_RUC.sh ~/PSC_cocul_day6_11s_ddSeq/s${i}_R1.fastq.gz ~/PSC_cocul_day6_11s_ddSeq/s${i}_R2.fastq.gz s${i} ~/Mouse_Genome_GRCm38.100/Musmu_GRC38_STAR_mappable_genome_10May2020 ~/Mouse_Genome_GRCm38.100/Musmu_GRCm38.100_protcod_fd.bed
  i=$((i+1))
done &
```
...
plot_knee(in_df = s1_RUC_long_df, cell_range = 1:200, vline_1 = 3, vline_2 = 48)
```
...
s1_RUC_long_df<-select_cells(s1_RUC_long_df, from = 3, to = 44)
s2_RUC_long_df<-select_cells(s2_RUC_long_df, from = 2, to = 120)
s3_RUC_long_df<-select_cells(s3_RUC_long_df, from = 4, to = 96)
s4_RUC_long_df<-select_cells(s4_RUC_long_df, from = 3, to = 204)
s5_RUC_long_df<-select_cells(s5_RUC_long_df, from = 11, to = 212)
s6_RUC_long_df<-select_cells(s6_RUC_long_df, from = 12, to = 252)
s7_RUC_long_df<-select_cells(s7_RUC_long_df, from = 9, to = 288)
s8_RUC_long_df<-select_cells(s8_RUC_long_df, from = 8, to = 226)
s9_RUC_long_df<-select_cells(s9_RUC_long_df, from = 6, to = 304)
s10_RUC_long_df<-select_cells(s10_RUC_long_df, from = 9, to = 210)
s11_RUC_long_df<-select_cells(s11_RUC_long_df, from = 7, to = 221)
```
...
PSC_cocul_11s_RUC_long<-bind_rows(s1_RUC_long_df, s2_RUC_long_df, s3_RUC_long_df, s4_RUC_long_df, s5_RUC_long_df, s6_RUC_long_df, s7_RUC_long_df, s8_RUC_long_df, s9_RUC_long_df, s10_RUC_long_df, s11_RUC_long_df)
```


The genes are re-ordered according to the gene_md table, i.e by chromosome position. Cell order is kept as descending frequency for samples s1-s11. The gene_ID is re-introduced. The code used to generate the gene_md table is described in R markdown `Musmu_GRCm38_Ens100_10May2020.Rmd`. 
```{r}
Sample_rank<-tibble(Sample=paste("s", 1:11, sep=""), sample_rank=1:11)

sorted_genes<-read.table("~/Mouse_genome_GRCm38.100/Musmu_GRCm38.100_protcod_fd_Gene_md.txt") %>% 
  select(EnsID=V1, Gene=V2) %>% 
  mutate(gene_rank=1:nrow(.))

PSC_cocul_11s_RUC_long<-PSC_cocul_11s_RUC_long %>% 
...
```{r}
write.table(PSC_cocul_11s_RUC_long, file = "PSC_cocul_11s_RUC_long.txt", quote = F, row.names = F, col.names = T, sep = "\t")
```
...
df_to_MEX(PSC_cocul_11s_RUC_long, "PSC_cocul_day6_11s_Raw_UMI_Count", paste(sorted_genes$EnsID, sorted_genes$Gene, sep="_"))
...
PSC_d6_11s_counts<-DropletUtils::read10xCounts(samples = "PSC_cocul_day6_11s_Raw_UMI_Count/")

colnames(PSC_d6_11s_counts)<-readLines(file.path("PSC_cocul_day6_11s_Raw_UMI_Count", "barcodes.tsv.gz"))

features<-read.delim(file.path("PSC_cocul_day6_11s_Raw_UMI_Count", "features.tsv.gz"), header = FALSE, stringsAsFactors = FALSE)

DropletUtils::write10xCounts(path = "PSC_cocul_day6_11s_Raw_UMI_Count.h5", x = counts(PSC_d6_11s_counts),  gene.id = features[,1], gene.symbol = features[,2], type = "HDF5")

# verify it can load 
SO_h5<-CreateSeuratObject(Read10X_h5("PSC_cocul_day6_11s_Raw_UMI_Count.h5"))

SO_h5@assays$RNA@counts
